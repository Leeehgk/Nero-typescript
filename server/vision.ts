...
function buildVisionInstruction(pergunta: string): string {
...
function buildVisionFailureMessage(error: unknown, screenshotPath: string, model: string): string {
  const rawMessage = error instanceof Error ? error.message : String(error);
  const normalized = normalizeText(rawMessage);

  const looksLikeVisionSupportIssue =
...
export async function analisarTelaAtual(client: OpenAI, model: string, pergunta: string): Promise<string> {
  ...
  const messages: ChatCompletionMessageParam[] = [
    {
      role: "system",
      content:
        "Voce e o Nero analisando uma captura da tela atual do usuario. Responda em portugues do Brasil, de forma curta, objetiva e honesta. Descreva apenas o que puder inferir da imagem. Se houver duvida, diga isso.",
    },
    {
      role: "user",
      content: [
