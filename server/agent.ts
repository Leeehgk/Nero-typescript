...
function hasExplicitActionIntent(userMessage: string): boolean {
...
function isToolAllowed(name: string, userMessage: string): boolean {
  if (!RISKY_TOOLS.has(name)) return true;
...
async function executeRecoveredToolCall(
  ...
  const result = await executarFerramenta(
    parsed.name as Parameters<typeof executarFerramenta>[0],
    parsed.args
  );
  return result;
}
  ...
  for (let step = 0; step < 15; step++) {
    ...
    if (tagMatch) {
      ...
      if (isToolAllowed(tagMatch.name, userMessage)) {
        const result = await executarFerramenta(
          tagMatch.name as Parameters<typeof executarFerramenta>[0],
          tagMatch.args
        );
