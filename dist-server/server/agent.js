...
function buildAgentSystemPrompt(nomeUsuario) {
...
function toPublicPendingApproval(approval) {
    return {
        id: approval.id,
...
function cleanFolderPhrase(value) {
...
function firstKnownFolderMention(userMessage) {
...
function extractRequestedFolderName(userMessage, nestedCue) {
    ...
    if (!nestedCue && knownFolder)
        return knownFolder;
    return undefined;
}
function extractBaseFolderHint(userMessage, nestedCue) {
    const normalized = normalizeText(userMessage);
    ...
    if (explicitBaseMatch) {
        ...
        if (candidate && !["ela", "dela"].includes(candidate))
            return candidate;
    }
    if (!nestedCue)
        return undefined;
...
function repairFolderToolArgs(args, userMessage) {
    const nestedCue = /\bdentro d[aeo]\b|\bdentro dela\b|\bsubpasta\b|\bpasta chamada\b|\bchamada\b/.test(normalizeText(userMessage));
    ...
    if (nome)
        ...
        delete nextArgs.nome;
    if (dentroDe)
        nextArgs.dentro_de = dentroDe;
    else
        delete nextArgs.dentro_de;
    return nextArgs;
}
function cleanVisionQuestion(value) {
    return value.replace(/\s+/g, " ").trim();
}
function isExplicitScreenshotRequest(userMessage) {
    const normalized = normalizeText(userMessage);
    return [
        "print",
        "screenshot",
...
function isScreenAnalysisRequest(userMessage) {
    const normalized = normalizeText(userMessage);
    const mentionsScreen = normalized.includes("tela") || normalized.includes("screen") || normalized.includes("monitor");
    if (!mentionsScreen)
        return false;
    return [
        "olha",
...
function repairScreenToolArgs(args, userMessage) {
...
function repairToolCall(name, args, userMessage) {
    const normalizedMessage = normalizeText(userMessage);
    let nextName = name;
    let nextArgs = { ...args };
    if (normalizedMessage.includes("pasta")) {
        if (name === "abrir_programa")
            nextName = "abrir_pasta";
        if (name === "fechar_programa")
            nextName = "fechar_pasta";
    }
...
function buildApprovalCopy(name, args) {
    switch (name) {
        case "tocar_youtube": {
        ...
        case "analisar_tela": {
            ...
            const genericQuestion = !pergunta ||
                ["olha a tela", "olhe a tela", "veja a tela", "ve a tela", "analisa a tela", "analise a tela"].includes(normalized);
            if (genericQuestion) {
                return {
...
function createPendingApproval(name, args, executionContext) {
...
function createPendingPlan(actions, executionContext) {
    cleanupPendingPlans();
...
function toPublicPendingPlan(plan) {
    return {
        id: plan.id,
        toolName: "agent_plan",
...
async function executeRecoveredToolCall(generated, userMessage, toolCallsNames, executionContext) {
    const parsed = parseTaggedToolCall(generated);
    if (!parsed)
        return null;
    const repaired = repairToolCall(parsed.name, parsed.args, userMessage);
...
export async function runAgentTurn(client, model, visionModel, nomeUsuario, perfil, userMessage) {
    ...
    const executionContext = { client, visionModel };
    for (let step = 0; step < 15; step++) {
        ...
        try {
            ...
            const recoveredResult = await executeRecoveredToolCall(extractFailedGeneration(error), userMessage, toolCallsNames, executionContext);
            if (recoveredResult) {
        ...
        if (tagMatch) {
            ...
            const repaired = repairToolCall(tagMatch.name, tagMatch.args, userMessage);
            toolCallsNames.push(repaired.name);
            if (requiresApproval(repaired.name)) {
                const approval = createPendingApproval(repaired.name, repaired.args, executionContext);
                return {
...
export async function resolvePendingApprovalDecision(approvalId, approved) {
    cleanupPendingApprovals();
    ...
    const reply = await executeTool(approval.toolName, approval.args, approval.executionContext);
    return {
        reply,
...
export async function runAgentModeTurn(client, model, visionModel, nomeUsuario, perfil, userMessage) {
    ...
    for (let step = 0; step < MAX_AGENT_ACTIONS && plannedActions.length < MAX_AGENT_ACTIONS; step++) {
        ...
        try {
            ...
            if (parsed) {
                ...
                plannedActions.push({ name: repaired.name, args: repaired.args, summary: copy.summary });
                break;
            }
            throw error;
        }
        ...
        if (msg.tool_calls?.length) {
            ...
            for (const tc of msg.tool_calls) {
                if (tc.type !== "function")
                    continue;
                if (plannedActions.length >= MAX_AGENT_ACTIONS)
                    break;
                const name = tc.function.name;
                let args = {};
                try {
...
async function resolvePendingPlanApproval(planId, approved) {
    cleanupPendingPlans();
...
export async function resolveAnyApproval(approvalId, approved) {
